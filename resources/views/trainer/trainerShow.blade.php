...
@section('content')
    <div class="min-h-screen ml-[5rem]  py-12">
        <div class="container mx-auto px-4">
            <div
                class="bg-white shadow-2xl rounded-3xl overflow-hidden transform hover:scale-[1.02] transition-all duration-300">
                <!-- Trainer Header -->
                        ...
                        <div class="relative mb-6 sm:mb-0 sm:mr-8">
                            <div class="w-40 h-40 rounded-full border-4 border-white shadow-lg overflow-hidden">
                                <img class="w-full h-full object-cover transform hover:scale-110 transition-all duration-300"
                                    src="{{ asset('storage/images/' . $trainer->image) }}" alt="{{ $trainer->name }}">
                            </div>
                ...
                <div class="p-8" x-data="{ tab: 'about' }">
                    <nav class="flex space-x-4 mb-8">
                        <button @click="tab = 'about'"
                            :class="{ 'text-[#0890b1] border-b-2 border-[#0890b1]': tab === 'about' }"
                            class="text-lg font-semibold pb-2 focus:outline-none transition-all duration-300">About</button>
                        <button @click="tab = 'sessions'"
                            :class="{ 'text-[#0890b1] border-b-2 border-[#0890b1]': tab === 'sessions' }"
                            class="text-lg font-semibold pb-2 focus:outline-none transition-all duration-300">Sessions</button>
                        <button @click="tab = 'testimonials'"
                            :class="{ 'text-[#0890b1] border-b-2 border-[#0890b1]': tab === 'testimonials' }"
                            class="text-lg font-semibold pb-2 focus:outline-none transition-all duration-300">Testimonials</button>
                    </nav>
                    ...
                    <div x-show="tab === 'about'" x-transition:enter="transition ease-out duration-300"
                        ...
                        <p class="text-gray-600 mb-6">
                            As a passionate fitness trainer, I'm dedicated to helping my clients achieve their health and
                            wellness goals. With a personalized approach to training, I ensure that each session is tailored
                                ...
                                <ul class="space-y-2">
                                    @foreach (['Strength Training', 'Weight Loss', 'Functional Fitness', 'Nutrition Coaching'] as $specialization)
                                        <li class="flex items-center">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-[#0890b1] mr-2"
                    ...
                    <div x-show="tab === 'sessions'" x-transition:enter="transition ease-out duration-300"
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            @endphp
                            @forelse($thetrainerSessions as $session)
                                <div
                                    ...
                                    <img class="h-48 w-full object-cover"
                                        src="{{ asset('storage/images/' . $session->image) }}" alt="{{ $session->name }}">
                                    <div class="p-4">
                                        <h3 class="font-bold text-lg text-[#0890b1] mb-2">{{ $session->name }}</h3>
                                        <div class="flex items-center text-gray-600 mb-2">
                                            ...
                                            <span>{{ \Carbon\Carbon::parse($session->start_time)->format('M d, Y H:i') }}</span>
                                        </div>
                                        <div class="flex items-center text-gray-600">
                                            ...
                                            <span>{{ \Carbon\Carbon::parse($session->end_time)->format('M d, Y H:i') }}</span>
                                        </div>
                                    </div>
                                </div>
                            @empty
                                <p class="col-span-full text-center text-gray-500">No upcoming sessions available.</p>
                            @endforelse
                        </div>
                    </div>

                    <div x-show="tab === 'testimonials'" x-transition:enter="transition ease-out duration-300"
                        x-transition:enter-start="opacity-0 transform scale-90"
                        x-transition:enter-end="opacity-100 transform scale-100">
                            ...
                            @php
                                $fakeTestimonials = [
                                    [
                                        'name' => 'John Doe',
                                        'comment' =>
                                            'Working with this trainer has been transformative. Ive seen significant improvements in my strength and overall fitness.',
                                    ],
                                    [
                                        'name' => 'Jane Smith',
                                        'comment' =>
                                            'The personalized workout plans and nutrition advice have helped me achieve my weight loss goals. Highly recommended!',
                                    ],
                                ];
                            @endphp
